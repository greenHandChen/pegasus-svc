...
public class UserServiceImpl implements IUserService {
    ...
    public UserVO getCurrentUser() {
        CuxUserDetails cuxUserDetails = SecurityHelper.getCurrentUser();
    ...
    public void createOrUpdateAccount(User user) {
        if (user != null && user.getId() == null) {
            ...
            user.setPassword(SecurityHelper.BCryptEncode(user.getPassword()));
        } else {
    ...
    public void modifyPassword(Long userId, String oldPassword, String newPassword) {
        ...
        userRepository.findById(userId).ifPresent(user -> {
            if (!SecurityHelper.BCryptMatch(oldPassword, user.getPassword())) {
                throw new CommonException("原密码不正确!");
            }
            user.setPassword(SecurityHelper.BCryptEncode(newPassword));
            userRepository.save(user);
        });
