...
angular.module('activitiModeler')
  .controller('StencilController', ['$rootScope', '$scope', '$http', '$modal', '$timeout', function ($rootScope, $scope, $http, $modal, $timeout) {
    ...
    $scope.editorFactory.promise.then(function () {
      ...
      $scope.editor.registerOnEvent(ORYX.CONFIG.EVENT_SELECTION_CHANGED, function (event) {
        ...
        if (shapes && shapes.length > 0) {
          ...
          /**
           * @Author: enHui.Chen
           * @Description: ehui.chen修改(元素的表单被修改或生成时的回调,可以自定义处理表单的值)(赋予主键默认值)
           * @Data 2019/10/24
           */
            // start
          ...
          if (selectedItem) {
            selectedItem.forEach(function (item) {
              if (item.key === 'oryx-overrideid' && !item.value) {
                item.value = 'sid-' + generateUUID();
                $scope.updatePropertyInModel(item);
              }
            });
          }
          // end
    ...
    /**
     * @Author: enHui.Chen
     * @Description: 设置候选人表单属性
     * @Data 2019/9/25
     */
    $scope.assignment = [];
    $scope.setAssignment = function (property) {
      var obj = {
        id: "APPROVE_CANDIDATE_APPROVER",
        name: "",
        expression: "",
        readable: true,
        type: "enum",
      ...
      if (property.value && property.value.assignment && property.value.assignment.length > 0) {
        ...
        for (var i = 0; i < assignment.length; i++) {
          ...
          if (code == "APPOINTED_EMPLOYEE") {
            type = "employeeCode"
          } else if (code == "APPOINTED_POSITION" || code == "CHOOSE_PERSON_POSITION") {
            type = "positionCode";
      ...
      $scope.syncFormProperties(obj, 'APPROVE_CANDIDATE_APPROVER');

    }

    /**
     * @Author: enHui.Chen
     * @Description: 同步候选人表单属性
     * @Data 2019/9/25
     */
