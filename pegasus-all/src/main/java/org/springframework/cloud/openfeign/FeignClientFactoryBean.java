...
import com.pegasus.common.openfeign.FeignGatewayClient;
import com.pegasus.common.openfeign.FeignGatewayProperties;
import feign.Client;
...
class FeignClientFactoryBean
    ...
    <T> T getTarget() {
        ...
        if (!StringUtils.hasText(this.url)) {
            ...
            /* enhui.chen start */
            FeignGatewayProperties feignGatewayProperties = this.applicationContext.getBean(FeignGatewayProperties.class);
            boolean enabled = feignGatewayProperties.isEnabled();
            String gatewayPath = feignGatewayProperties.getGatewayPath();

            if(enabled && gatewayPath != null && !gatewayPath.equals("")){
                this.url = !gatewayPath.startsWith("http")? "http://" + gatewayPath:gatewayPath;
            }else{
                this.url = !this.name.startsWith("http")?"http://" + this.name:this.name;
            }
            this.url += cleanPath();

            HardCodedTarget<?> hardCodedTarget = new HardCodedTarget<>(this.type, this.name, this.url);
            if (enabled) {
                // service转换url
                FeignGatewayClient feignGatewayClient = this.applicationContext.getBean(FeignGatewayClient.class);
                builder.client(feignGatewayClient);
                Targeter targeter = get(context, Targeter.class);
                return (T) targeter.target(this, builder, context, hardCodedTarget);

            } else {

                // service模式
                return (T) loadBalance(builder, context, hardCodedTarget);
